...
async def get_user_by_token(token: str = Depends(API_KEY_HEADER),
...
async def user_follow(user_id: int,
    ...
    if await check_user_follow(session=session, follower_id=follower.id, following_id=user_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def user_unfollow(user_id: int,
    ...
    if not await check_user_follow(session=session, follower_id=follower.id, following_id=user_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def check_user_follow(follower_id: int,
                            following_id: int,
                            session: AsyncSession = Depends(get_async_session)
                            ) -> bool:
